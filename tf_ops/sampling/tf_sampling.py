...
def farthest_point_sample(npoint, inp, global_idx):
    '''
    input:
        int32
        batch_size * ndataset * 3   float32
    ...
    returns:
        ...
        batch_size * npoint         int32
    '''
    return sampling_module.farthest_point_sample(inp, global_idx, npoint)
...
if __name__=='__main__':
    ...
    with tf.device('/gpu:0'):
        ...
        # pt_sample = tf.zeros(shape=pt_sample.shape, dtype=tf.int32)
        print('pt_sample: ', pt_sample)
        global_idx = tf.zeros(shape=(1,8192), dtype=tf.int32)
        print('global_idx: ', global_idx)
        idxs, g_idxs = farthest_point_sample(1024, pt_sample, global_idx)
        reduced_sample =gather_point(pt_sample,idxs)

...
REGISTER_OP("FarthestPointSample")
  ...
  .SetShapeFn([](::tensorflow::shape_inference::InferenceContext* c) {
    ...
    ::tensorflow::shape_inference::ShapeHandle dims2; // batch_size * npoints
    c->WithRank(c->input(1), 2, &dims2);
    ::tensorflow::shape_inference::ShapeHandle output0 = c->MakeShape({c->Dim(dims1, 0), npoint});
    ::tensorflow::shape_inference::ShapeHandle output1 = c->MakeShape({c->Dim(dims1, 0), npoint});
    c->set_output(0, output0);
...
void farthestpointsamplingLauncher(int b,int n,int m,const int * g_idx, const float * inp,float * temp,int * out,int * out_g_idx);
class FarthestPointSampleGpuOp: public OpKernel{
  public:
    ...
    void Compute(OpKernelContext * context)override{
      int m = npoint_;
      const Tensor& inp_tensor=context->input(0);
      const Tensor& global_idx=context->input(1);
//      printf("inp_tensor:", inp_tensor.shape());
//      printf("global_idx:", global_idx.shape());
      ...
      OP_REQUIRES(context,global_idx.dims()==2, errors::InvalidArgument("FarthestPointSample expects (batch_size,num_points) global_idx shape"));
      OP_REQUIRES(context,inp_tensor.shape().dim_size(0)==global_idx.shape().dim_size(0) && inp_tensor.shape().dim_size(1)==global_idx.shape().dim_size(1), errors::InvalidArgument("FarthestPointSample expects equal first 2 dim of inp and global_idx"));
      int b=inp_tensor.shape().dim_size(0);
      int n=inp_tensor.shape().dim_size(1);
      auto inp_flat=inp_tensor.flat<float>();
      auto global_idx_flat=global_idx.flat<int>();
      const float * inp=&(inp_flat(0));
      const int * g_idx=&(global_idx_flat(0));
      Tensor * out_tensor;
      Tensor * out_global_idx;
      OP_REQUIRES_OK(context,context->allocate_output(0,TensorShape{b,m},&out_tensor));
      OP_REQUIRES_OK(context,context->allocate_output(1,TensorShape{b,m},&out_global_idx));
      auto out_flat=out_tensor->flat<int>();
      auto out_g_idx_flat=out_global_idx->flat<int>();
      int * out=&(out_flat(0));
      int * out_g_idx=&(out_g_idx_flat(0));
      Tensor temp_tensor;
      OP_REQUIRES_OK(context,context->allocate_temp(DataTypeToEnum<float>::value,TensorShape{32,n},&temp_tensor));
      auto temp_flat=temp_tensor.flat<float>();
      float * temp=&(temp_flat(0));
      farthestpointsamplingLauncher(b,n,m,g_idx,inp,temp,out,out_g_idx);
    }
    private:
        int npoint_;
};

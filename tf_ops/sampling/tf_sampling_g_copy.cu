...
__global__ void farthestpointsamplingKernel(int b,int n,int m,const int * __restrict__ g_idx, const float * __restrict__ dataset,float * __restrict__ temp,int * __restrict__ idxs, int * __restrict__ out_g_idx){
  if (m<=0)
    return;
...
void farthestpointsamplingLauncher(int b,int n,int m,const int * g_idx, const float * inp,float * temp,int * out,int * out_g_idx){
  farthestpointsamplingKernel<<<32,512>>>(b,n,m,g_idx,inp,temp,out,out_g_idx);
}

...
TRAINING_FILE_LIST = os.path.join(ROOT_DIR, TRAINING_FILE_LIST)
# Load ALL data
...
for h5_filename in train_file_list:
    h5_filename = os.path.join(ROOT_DIR, h5_filename)
...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(GPU_INDEX)):
            ...
            # Get model and loss 
            pred_sem, end_points = get_model(pointclouds_pl, NUM_CLASSES, is_training_pl, extra_constraint=True, bn_decay=bn_decay)
            loss, classify_loss, sim_map_loss = get_loss(pred_sem, sem_labels_pl, end_points, is_training_pl)
...
def train_one_epoch(sess, ops, train_writer):
    ...
    log_string('----')
    current_data, current_sem, _ = provider.shuffle_data(train_data[:, 0:NUM_POINT, :], train_sem)
    ...
    for batch_idx in range(num_batches):
        ...
        summary, step, _, loss_val, classify_loss_val, sim_map_loss_val = sess.run([ops['merged'], ops['step'], ops['train_op'], ops['loss'], ops['classify_loss'], ops['sim_map_loss']],
                                         feed_dict=feed_dict)
        train_writer.add_summary(summary, step)
        loss_sum += loss_val
        classify_loss_sum += classify_loss_val
        if batch_idx % 50 == 0:
            log_string("loss: {:.2f}; classfify_loss: {:.2f}; sim_map_loss: {:.2f}".format(loss_val, classify_loss_val, sim_map_loss_val))
    
    log_string('mean loss: %f' % (loss_sum / float(num_batches)))
    log_string('mean classify_loss: %f' % (classify_loss_sum / float(num_batches)))
